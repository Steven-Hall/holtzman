...
class CompilationErrorHandlingTests:
    ...
    def test_invalid_escape_sequence(self):
        with pytest.raises(TemplateError) as error:
            holtzman.from_string("this is \\a invalid")
    ...
    def test_missing_closing_brace(self):
        with pytest.raises(TemplateError) as error:
            holtzman.from_string("12345{{ variable }")
    ...
    def test_invalid_variable_name_in_for_loop(self):
        with pytest.raises(TemplateError) as error:
            holtzman.from_string("{% for invalid.variable in variables %}{% end %}")
    ...
    def test_invalid_for_loop(self):
        with pytest.raises(TemplateError) as error:
            holtzman.from_string("12345{% for invalid variables %}{% end %}")
    ...
    def test_invalid_template_string(self):
        with pytest.raises(TemplateError) as error:
            holtzman.from_string("12345{% invalid statement %}{% end %}")
